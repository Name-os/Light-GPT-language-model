...
def merge(int_list, target_pair, replace):
    new_list = []
    i = 0
...
raw_values = list(map(int, text.encode("utf-8")))

def show_pairs(raw_values):
    # shows all times a pair appears and sorted
    new = {}
    stats = get_stats(raw_values)
    for k,v in stats.items():
        new[v] = k
    print(sorted(new.items(), key=lambda t: t[0], reverse=True))

    print(sorted(((v,k) for k,v in stats.items()), reverse=True))

def max_merges(raw_values):
    # does as many merges as possible
    merge_list = list(raw_values)

    merges = {}
    i = 0
    while True:
        stats = get_stats(merge_list)
        if len(merge_list) == 1:
            break
        top_pair = max(stats, key=stats.get)
        new_token_id = 256 + i
        merge_list = merge(merge_list, top_pair, new_token_id)
        print(f"{top_pair} -> {new_token_id}")
        merges[top_pair] = new_token_id

        i += 1

    return merges, merge_list

def normal_merge(raw_values, target_size):
...
merges, merge_list = normal_merge(raw_values, target_size)